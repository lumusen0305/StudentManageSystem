...
_oTop.loadJsFileToTop(["$js_path$qmplayer/player38b933.js","$js_path$com/kits/qmpreviewer/js/qmpreviewer493dc5.js"]);
_oTop.waitFor(function(){
...
try{
var _oExistList=_oAttach.getExistList(),_oTimeObj={cgitm:_oTop.g_cgiTimeStamp||0,clitm:_oTop.g_clientTimeStamp||0,comtm:_oTop.gSendTimeStart.valueOf(),logattcnt:_oExistList.length,logattsize:_oAttach.getAttachSize(),logattmethod:_oExistList.length?_oExistList[_oExistList.length-1].sUploadMode:"",timezone:-new Date(2009,0,1).getTimezoneOffset()/60*3600,timezone_dst:new Date(2009,0,1).getTimezoneOffset()/60*3600-new Date().getTimezoneOffset()/60*3600};
...
_oCompose.setNeedCloseConform(_asTemplate==='note_save'||_asTemplate==='note_autosave'||_asTemplate=="autosave"&&_oCompose.getPageConfig().subtmpl!="draft");
...
{
_oTop.loadJsFileToTop(["$js_path$com/kits/qmeditor/qqmail/release/editor493ddf.js"]);
...
_oTop.loadJsFileToTop(["$js_path$composeresp4ee49a.js"]);
},0);
...
}}}).initialize(_aoConfig.source||_outputDataLoading(),false,_oCompose.S("QMEditorArea").getAttribute("tIndex"),_oCompose.S("QMEditorArea").getAttribute("accKey"),_oCompose.S("QMEditorArea").getAttribute("title"));
...
_oCompose.loadValue(true);
_oCompose.setNeedCloseConform(true);
debug(['initEditorForNotePage, start autoSave.',new Date()]);
_oCompose.autoSave(false);
_oCompose._setEditorPageEvent();
_oCompose._clearEditorCheckTimer();
typeof (_aoConfig.onload)=="function"&&_aoConfig.onload(_oCompose._getEditorFocusStatus());
...
_oCompose.S('QMEditorArea').style.overflow='hidden';
...
_oRightAreaBtn.innerHTML='<input tabindex="0" type="button" hidefocus class='+(!_abShow?'nextfd ':'prefd ')+'aria-label='+(!_abShow?'\u663E\u793A\u901A\u8BAF\u5F55':'\u9690\u85CF\u901A\u8BAF\u5F55')+' />';
if(_abShow)
{
_oRA.style.left=0;
...
debug(['setNeedCloseConform',_abNeed]);
...
var _oWin=_oCompose.getWin();
var _sExitCode;
debug(['closePage','isNeedCloseConform',_oCompose._isNeedCloseConform(),'ExitCode',_oCompose._confirmCheckBeforeClose()]);
...
var contentEdited=_oCompose._isContentEdited();
var enableAutoSave=_oCompose._isEnabledAutoSave();
var executeAutoSave=_abEnable&&contentEdited&&enableAutoSave;
debug(['autoSave execute:',executeAutoSave,'enable',_abEnable,'edited',_oCompose._isContentEdited(),'enableAutoSave',enableAutoSave,new Date()]);
if(executeAutoSave)
{
_oCompose.getPageId()!="note"?_oProcess.doProcess("autosave","save"):_oProcess.doProcess("note_autosave","save",true);
}
_oCompose.getComposeVar()._oAutoSaveTimer=_oWin.setTimeout(function(){
_oCompose.autoSave(true);
},_oCompose.getPageId()=="note"?30000:30000);
...
var _sFunc=_oTop.TE([_oTop.gbIsIE||_oTop.gbIsFF?'<input hideFocus type="button"  onmousedown="return false" unselectable="on" style="margin:2px 5px 0;" class="ico_att"/>\u62D6\u62FD\u4E0A\u4F20\u9644\u4EF6<br>':'',_bIsNewSC?'':'<input hideFocus type="button"  style="margin:2px 5px 0;" class="ico_attbig"/>\u8D85\u5927\u9644\u4EF6<br>','<input hideFocus style="background: url($images_path$/newicon/compose3bcbca.png) no-repeat 0 -120px;margin:2px 5px 0;height:16px;width:16px;padding:0;border:0;" type=button unselectable="on">\u622A\u5C4F']).replace({images_path:_oTop.getPath("image",true)}),_sMsg=_bIsUpdate?_oTop.T('<b>\u60A8\u7684$control$\u7248\u672C\u8FC7\u4F4E\uFF0C\u8BF7\u5347\u7EA7\u5230\u65B0\u7684\u7248\u672C\u3002</b>'):_oTop.T('<b>\u60A8\u8FD8\u672A\u5B89\u88C5$control$\u3002</b>'),_sCtrlName="\u622A\u5C4F",_nWidth=400,_nHeight=210;
switch(_asType)
{case "paste":
