...
},signtureCard:function(){
var _oSelf=this,_oWin=_oSelf._moWin,_oTop=getTop(),signtureCardDoms;
var catemailtype='';
var mailcontainerDom=_oTop.S('qqmail_mailcontainer',_oWin.document);
_oTop.addEvent(mailcontainerDom,'click',function(event){
var _target=event.srcElement||event.target;
var target=_getNode(_target);
if(target)
{
_oTop.preventDefault(event);
_addEventForSigntureCardDom(target);
}
});
function _getNode(node)
{
if(node.className==='xm_write_card')
{
return node;
}
else{
if(node.parentNode&&node.parentNode.nodeName&&node.parentNode.nodeName.toLowerCase()!=='body')
{
...
function _addEventForSigntureCardDom(_signtureCardDom)
{
var _href=_signtureCardDom.getAttribute('href');
var _code=_href.match(/code=(.+)$/)[1];
var _sUrl='/cgi-bin/xmail?xmmodule=addr&xmcgi=get_profile&func=5&code='+_code;
_oTop.QMAjax.send(_sUrl,{method:"POST",onload:function(_abIsOk,_asResult){
var res=_oSelf.handerXmailRespont(_asResult);
if(_abIsOk&&res)
{
var _oData=res.profiles.main_profile;
_hadSavedToContact(_oData);
}
}});
}
function _hadSavedToContact(_oData)
{
var _sUrl='/cgi-bin/laddr_list?operate=view&t=contact.json&view=normal';
_oTop.QMAjax.send(_sUrl,{method:"POST",onload:function(_abIsOk,_asResult){
_asResult=_oTop.evalValue(_asResult);
if(_abIsOk)
{
var _contactList=_asResult.oList;
var _flag=false;
var _contactItem=null;
for(var index=0;index<_contactList.length;index++)
{
_contactItem=_contactList[index];
var _oCusToms=_contactItem.oCustom;
for(var nCustomIndex=0;nCustomIndex<_oCusToms.length;nCustomIndex++)
{
var _oCusTomItem=_oCusToms[nCustomIndex];
if(_oCusTomItem.sLabel==='qqmail_profile_id'&&_oCusTomItem.sVal===_oData.profile_id)
{
_flag=true;
break;
}
...
if(_flag)
{
_oSelf.showContactDialog(_contactItem);
}
else{
_oSelf.showSigntureCardDialog(_oData);
}
}
}});
}
},showContactDialog:function(_oContactItem){
var _oSelf=this,_oTop=getTop();
var _sUrl=T("/cgi-bin/laddr_detail?sid=$sid$&view=normal&t=contact&AddrID=$addrid$&dlgname=infoAddr").replace({sid:_oTop.getSid(),addrid:_oContactItem.sId});
var _oAddrDlg=new _oTop.QMDialog({sId:"infoAddr",sTitle:"\u7F16\u8F91\u8054\u7CFB\u4EBA",sUrl:_sUrl,nHeight:538,nWidth:600,onload:function(){
this.S("_dlgiframe_").scrolling="yes";
}});
},handerXmailRespont:function(res){
var _res=null;
try{
...
if(_res.head.ret!==0)
{
var RetCode=+_res.head.ret;
var errorMsg='\u63A5\u53E3\u8BF7\u6C42\u5931\u8D25\uFF0C\u9519\u8BEF\u7801:'+RetCode;
return null;
}
else{
return _res.body;
...
},showSigntureCardDialog:function(_oData){
var _oSelf=this,_oWin=_oSelf._moWin,_oTop=getTop();
var _oSingtureCardDlg=new _oTop.QMDialog({sId:"singtureCard",sTitle:"\u8054\u7CFB\u4EBA",sBodyHtml:_oTop.TE(['<div class="signture_wrapper">','<div class="signture_container">','<div class="signture_container_header">','<div class="signture_container_header_item">','<div class="signture_container_header_item_label signture_container_header_item_avatar"><img src="$avatarUrl$"></div>','<div class="signture_container_header_item_name"><%@WmlEncode($name$)%></div>','</div>','<div class="signture_container_header_item">','<div class="signture_container_header_item_label">\u90AE\u7BB1</div>','<div class="signture_container_header_item_text">$mail$</div>','</div>','$@$if($otherInfos$)$@$','<div class="signture_container_header_info">','$@$for($otherInfos$)$@$','<div class="signture_container_header_item">','<div class="signture_container_header_item_label">&nbsp;</div>','<div class="signture_container_header_item_text">$value$</div>','</div>','$@$endfor$@$','</div>','$@$endif$@$','</div>','</div>','</div>']).replace({avatarUrl:_oData.icon,name:_oData.name,mail:_oData.mail,otherInfos:_oData.other_infos||null}),sFootHtml:'<a id="saveBtn" class="btn_blue btn_space" href="javascript:;">\u4FDD\u5B58\u8054\u7CFB\u4EBA</a><a class="btn_gray" id="composeBtn" href="javascript:;">\u5199\u4FE1</a>',nWidth:500,onload:function(){
var saveBtnDom=this.S("saveBtn");
var _oSelf=this;
saveBtnDom.onclick=function(event){
_saveSigntureCardToContact(event);
};
var composeBtnDom=this.S("composeBtn");
composeBtnDom.onclick=function(event){
_oSelf.close();
_oTop.openComposeDlg("normal",{sDefAddrs:_oData.mail,bUinEncrypt:true,bAddrEdit:true,nWidth:490,sDefSubject:""});
};
}});
function _saveSigntureCardToContact(e)
{
var _sUrl='/cgi-bin/laddr_addedit?&operate=save&t=contact.json&ef=js&resp_charset=UTF8';
var _aoParam={'name':_oData.name,'email':_oData.mail,'photo':_oData.icon,'qqmail_profile_id':_oData.profile_id,'qqmail_profile_icon':_oData.icon,'qqmail_profile_name':_oData.name,'qqmail_profile_email':_oData.mail};
if(_oData.other_infos&&_oData.other_infos.length>0)
{
for(var otherInfoIndex=0;otherInfoIndex<_oData.other_infos.length;otherInfoIndex++)
{
var otherInfoIndexNext=otherInfoIndex+1;
_aoParam['qqmail_profile_info'+otherInfoIndexNext]=_oData.other_infos[otherInfoIndex].value;
}
}
_oTop.QMAjax.send(_sUrl,{method:"POST",content:_serialize(_aoParam),onload:function(_abIsOk,_asResult){
if(_abIsOk)
{
_oTop.showInfo("\u4FDD\u5B58\u5230\u901A\u8BAF\u5F55\u6210\u529F");
_oSingtureCardDlg.close();
}
else{
_oTop.showError("\u4FDD\u5B58\u5230\u901A\u8BAF\u5F55\u5931\u8D25");
_oSingtureCardDlg.close();
}
}});
}
function _serialize(_aoParam)
{
var _oOuts=[];
for(var key in _aoParam)
{
if(key.indexOf('qqmail_profile_')>-1)
{
_oOuts.push(['custom',key+':'+encodeURIComponent(_aoParam[key])].join("="));
}
else{
_oOuts.push([key,encodeURIComponent(_aoParam[key])].join("="));
}
}
return _oOuts.join("&");
}
...
loadCssFile("$css_path$../js/com/kits/qmpreviewer/css/previewer493ddf.css",true,_oTop.document);
...
var replyName,replyAddr;
if(_oMail.replyTo&&_oMail.replyTo.length)
{
E(_oMail.replyTo,function(reply){
if(reply.addr===_oMail.from.addr)
{
replyName=reply.name;
replyAddr=reply.addr;
return false;
}
});
}
if(!replyName)
replyName=_oMail.from.name;
if(!replyAddr)
replyAddr=_oMail.from.addr;
var _sTmpInclude=_TEMPLATE._REFERPART.replace(extend({},_oMail,_oConfig.titlePrefix=="1"?_LANG._EN_US:_LANG._ZH_CN,{'name':replyName,'addr':replyAddr}));
...
}}).initialize(_sInitEditorContent,false,_oSelf.S("QMEditorArea").getAttribute("tIndex"),false,"\u5FEB\u6377\u56DE\u590D");
}
else{
_oEditorDom.view("show");
...
var foldDom=_oSelf.S('fold');
adjustLocalTime(_oSelf.attr(foldDom,"local"),_oSelf.attr(foldDom,"remote"),_oSelf.S('local-time-caption-next-'));
...
setTimeout(function(){
var signtureCardDoms=_oTop.CN('xm_write_card',_oWin.document);
for(var i=0;i<signtureCardDoms.length;i++)
{
var _signtureCardDom=signtureCardDoms[i];
_oTop.addEvent(_signtureCardDom,'click',function(event){
_oTop.preventDefault(event);
});
}
},100);
...
function showMeetingReplyDialog(attendStatus,schedule,callback)
{
getTop().confirmBox({id:"meeting-reply-dialog",msg:'<div class="qui_txtDef">\u9700\u8981\u53D1\u9001\u7B54\u590D\u56DE\u6267\u7ED9\u53D1\u8D77\u4EBA\u5417\uFF1F</div>',title:"\u662F\u5426\u53D1\u9001\u7B54\u590D\u56DE\u6267",confirmClass:"qui_btn_Blue",onreturn:callback});
}
function startReplying(mailid,email,uid,calId,attendStatus,shouldFeedback)
{
doXmailRequest('/calendar/reply',{cal_id:{relative_id:mailid,id:calId,uid:uid},attendee:{name:getTop().nn,email:email,status:attendStatus},no_response:!shouldFeedback});
}
function replyInvitation(attendStatus,schedule,mailid,email,statusUiList)
{
showMeetingReplyDialog(attendStatus,schedule,function(shouldFeedback){
startReplying(mailid,email,schedule.uid,schedule.id||'',attendStatus,shouldFeedback);
var undecided=statusUiList[0],tentative=statusUiList[1],accepted=statusUiList[2],rejected=statusUiList[3];
undecided.style.display='none';
if(attendStatus===2)
tentative.style.display='inline-block';
if(attendStatus===4)
...
_aoLoadWin.replyInvitation=replyInvitation;
function timestampToTime(timestamp,shouldRemovePrefix)
{
var date=new Date(timestamp*1000),Y=date.getFullYear(),M=date.getMonth()+1,D=date.getDate(),h=date.getHours(),m=date.getMinutes();
var weekDay='\uFF08\u661F\u671F'+"\u65E5\u4E00\u4E8C\u4E09\u56DB\u4E94\u516D".charAt(date.getDay())+'\uFF09';
var isToday=new Date(timestamp*1000).toDateString()===new Date().toDateString();
var prefix=shouldRemovePrefix?'':(isToday?'\u4ECA\u5929':Y+'\u5E74'+M+'\u6708'+D+'\u65E5'+weekDay);
return prefix+(h<12?'\u4E0A\u5348':'\u4E0B\u5348')+(h<=12?h:h-12)+' : '+(m<10?'0'+m:m);
}
function resolveCalendar(mailId,calStr,callback)
{
if(!mailId||!calStr||!callback)
{
return;
}
doXmailRequest('calendar/parser',{relative_id:mailId,vcalendar:calStr},function(success,body){
if(success&&body&&body.schedule)
{
var schedule=body.schedule;
var summary=schedule.subject.length<=40?schedule.subject:schedule.subject.substring(0,40)+'\u2026';
var desc=timestampToTime(schedule.start_time)+' - '+timestampToTime(schedule.end_time,new Date(schedule.start_time*1000).toDateString()===new Date(schedule.end_time*1000).toDateString())+(schedule.location?', ':'')+(schedule.location.length<=40?schedule.location:schedule.location.substring(0,40)+'\u2026');
callback(summary,desc,schedule);
}
});
}
_aoLoadWin.resolveCalendar=resolveCalendar;
})(window);
(function(){
});
